...
void ObtainModelInstance_t(CModelInstance& mi, const CTString& fnSmcFile)
{
    ASSERT(&mi != NULL);

    CModelInstanceData* pmid = _pModelInstanceStock->Obtain_t(fnSmcFile);
    ASSERT(pmid != NULL && pmid->mid_pModelInstance != NULL);

    mi.Copy(*pmid->mid_pModelInstance);
    _pModelInstanceStock->Release(mi.mi_pmidData);
}
...
void CModelInstance::Copy(CModelInstance& miOther)
{
    ...
    {
        mi_pmidData = miOther.mi_pmidData;
        mi_pmidData->MarkUsed();
    }
...
{
    if (mi_pmidData != NULL) {
        _pModelInstanceStock->Release(mi_pmidData);
        mi_pmidData = NULL;
    }
...
CModelInstanceData::CModelInstanceData()
{
    mid_pModelInstance = NULL;
}

CModelInstanceData::~CModelInstanceData()
{
    if (mid_pModelInstance != NULL) {
        if (mid_pModelInstance->mi_pmidData != NULL)
        {
            mid_pModelInstance->mi_pmidData->ser_ctUsed = 0;
        }

        mid_pModelInstance->mi_pmidData = NULL;
        DeleteModelInstance(mid_pModelInstance);
        mid_pModelInstance = NULL;
...
void CModelInstanceData::Read_t(CTStream* istrFile)
{
    ...
    mid_pModelInstance->mi_pmidData = this;
    ASSERT(mid_pModelInstance != NULL);
    // Remeber pointer to model instance serial from model instance
}

void CModelInstanceData::Write_t(CTStream* ostrFile)
{
    ASSERTALWAYS("CModelInstanceData cannot be writen");
}

void CModelInstanceData::Clear(void)
{
    ASSERTALWAYS("CModelInstanceData cannot be cleared");
}

SLONG CModelInstanceData::GetUsedMemory(void)
{
    if (mid_pModelInstance == NULL) {
        return sizeof(*this);
    }
    else {
        return sizeof(*this) + mid_pModelInstance->GetUsedMemory();
    }
}

CTString CModelInstanceData::GetDescription(void)
{
    return CTString("");
}
